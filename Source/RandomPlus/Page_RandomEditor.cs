...
namespace RandomPlus
{
    public class Page_RandomEditor : Page
    {
        ...
        public override void DoWindowContents(Rect inRect)
        {
            ...
            var randomRerollLimitRect = new Rect(320 + 20, 156 + 40 + 20 + 20 + 4, 70, 30);
            for (int i=0; i<RandomSettings.RANDOM_REROLL_LIMIT_OPTIONS.Length; i++)
            {
                int option = RandomSettings.RANDOM_REROLL_LIMIT_OPTIONS[i];
                ...
                randomRerollLimitRect.x += randomRerollLimitRect.width + 10;
                //randomRerollLimitRect.width += 6;

                if ((i + 1) % 4 == 0) {
                    randomRerollLimitRect.x = randomRerollLimitLabelRect.x;
                    randomRerollLimitRect.y += randomRerollLimitLabelRect.height + 10;
                }
            }

            var ageLabelRect = randomRerollLimitLabelRect.OffsetBy(0, 88);
            ageLabelRect.height = 26;
            string labelText = string.Format("Age: {0}-{1}", 
                0, //RandomSettings.PawnFilter.AgeRange.min, 
